        ...
        methods: {
            ...
            handleFilterSelect (index, value) {
            	if(this.filterServer){
					...
					var _key = this.cloneColumns[index].key;
					for(var i = 0;i<this.filterColumns.length;i++){
						if(this.filterColumns[i].key == _key){
							this.filterColumns[i].value = value;
							ishasfilter = true;
							break;
						}
					}
					if(!ishasfilter){
	            		this.filterColumns.push({"key":_key,"value":value});
					}
                	this.cloneColumns[index]._isFiltered = true;
                	this.cloneColumns[index]._filterVisible = false;
            	}else{
            ...
            handleFilterReset (index) {
            	if(this.filterServer){
					...
					if(_filterindex!=-1){
	            		this.filterColumns.splice(_filterindex,1);
					}
            	}else{
	                this.cloneColumns[index]._isFiltered = false;
	                this.cloneColumns[index]._filterVisible = false;
